---
layout: post
title: "Intelligence into Urban Mobility"
date: 2010-02-19 20:54:40 +0100
categories: []
tags: ['assistant de mobilité', 'Ertico', 'Infrastructure', 'TIC', 'VE', 'Véhicule']
---

ERTICO organisait Le 4/02 un workshop sur la **[mobilité urbaine et l'apport des TIC](http://www.ertico.com/en/events/past_events/intelligence_into_urban_mobility_breakfast_workshop.htm)**. 

 

ITS Europe held its second breakfast workshop at the European Parliament on the theme “Intelligence into Urban Mobility”. Co-hosted by Dieter-Lebrecht Koch MEP and Zita Gurmai MEP, the event boasted high level speakers from industry and the European institutions. ERTICO strongly supports its Partners’ and European initiatives in advancing and deploying ITS solutions to make Urban Mobility safer, greener, more economical and more efficient. This needs an approach which involves all stakeholders and activity areas: infrastructure, behaviour and vehicles. A focus on cost-effective and mature solutions and a coordinated implementation is vital.

 

Click [here](http://intelligentmobility.wordpress.com/) to access continue the debate on the ERTICO blog.

 

Retrouver ci dessous les présentations concernant les véhicules, l'infrastructure et l'information apportéee pour devenir the connected traveller" :

 

 

 
 
- Christian Rousseau, Renault - Electrification of vehicles and ITS 
- Hans-Joachim Schade, Siemens - Intersection safety for cities  
- Jan van der Wel, Technolution - Multimodal urban mobility services: the connected traveller, here and now! [Technolution]() View more [presentations]() from [transportsdufutur](). [Siemens]() View more [presentations]() from [transportsdufutur]().
 [Renault]() View more [presentations]() from [transportsdufutur]()."
